...
// selfUpdate checks for and applies a julian-ai update before normal dispatch.
...
func selfUpdate(ctx context.Context, version string, profile system.PlatformProfile, stdout io.Writer) error {
	...
	// Check for updates (only julian-ai).
	results := updateCheckFiltered(ctx, version, profile, []string{"julian-ai"})

	// Find the julian-ai result.
	var target *update.UpdateResult
	for i := range results {
		if results[i].Tool.Name == "julian-ai" {
			target = &results[i]
			break
		}
	}
	...
	for _, r := range report.Results {
		if r.ToolName == "julian-ai" && r.Status == upgrade.UpgradeSucceeded {
			succeeded = true
			break
		}
	}
	...
	//
	// Use exec.LookPath("julian-ai") rather than os.Executable() because
	// on Homebrew, os.Executable() resolves to the versioned Cellar path
	// (e.g. /opt/homebrew/Cellar/julian-ai/1.8.5/bin/julian-ai) which
	// still points to the OLD binary after upgrade. The PATH symlink
	// (/opt/homebrew/bin/julian-ai) is updated by Homebrew to the new
	// version, so LookPath gives us the correct binary.
	executable, err := lookPathFn("julian-ai")
	if err != nil {
