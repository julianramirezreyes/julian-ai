...
// RunRestore is the top-level entry point for `julian-ai restore [args]`.
...
func runRestoreWithHomeDir(args []string, restorer RestoreFunc, stdout io.Writer, stdin io.Reader, homeDir string) error {
	...
	if len(positional) == 0 {
		return fmt.Errorf("usage: julian-ai restore [--list | latest | <id>] [--yes]")
	}
...
// and the julian-ai version that created the backup when known.
...
func resolveRestoreTarget(target string, backups []backup.Manifest) (backup.Manifest, error) {
	...
	return backup.Manifest{}, fmt.Errorf("backup %q not found — use `julian-ai restore --list` to see available backups", target)
}
...
func backupRootDir(homeDir string) string {
	return homeDir + "/.julian-ai/backups"
}
