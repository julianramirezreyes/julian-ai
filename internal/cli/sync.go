...
import (
	...
	"github.com/julianramirezreyes/julian-ai/internal/components/gga"
	"github.com/julianramirezreyes/julian-ai/internal/components/mcp"
	"github.com/julianramirezreyes/julian-ai/internal/components/permissions"
	...
	"github.com/julianramirezreyes/julian-ai/internal/components/theme"
...
//
// Discovery order:
//  1. Persisted state (~/.julian-ai/state.json) — written at install time.
...
func RenderSyncReport(result SyncResult) string {
	...
	if result.NoOp {
		fmt.Fprintln(&b, "julian-ai sync — no managed sync actions needed")
	...
	if result.DryRun {
		fmt.Fprintln(&b, "julian-ai sync — dry-run")
		fmt.Fprintf(&b, "Agents: %s\n", joinAgentIDs(result.Agents))
	...
	fmt.Fprintln(&b, "julian-ai sync — managed sync executed")
