...
func TestExecute_NoopWhenNothingIsExecutable(t *testing.T) {
	results := []update.UpdateResult{
		makeResult("julian-ai", update.UpToDate, "1.0.0", "1.0.0", update.InstallBinary),
...
func TestExecute_DevBuildOnlyNoBackupCreated(t *testing.T) {
	...
	results := []update.UpdateResult{
		makeResult("julian-ai", update.DevBuild, "dev", "1.0.0", update.InstallBinary),
	}
...
// julian-ai with DevBuild status must appear in Results as UpgradeSkipped
...
func TestExecute_DevBuildIsSkipped(t *testing.T) {
	...
	// julian-ai (DevBuild) MUST appear as UpgradeSkipped with a ManualHint.
	var devResult *ToolUpgradeResult
	for i := range report.Results {
		if report.Results[i].ToolName == "julian-ai" {
			r := report.Results[i]
			devResult = &r
		}
	}
	if devResult == nil {
		t.Fatalf("julian-ai (DevBuild) must appear in Results — was not found")
	}
	if devResult.Status != UpgradeSkipped {
		t.Errorf("julian-ai DevBuild Status = %q, want UpgradeSkipped", devResult.Status)
	}
	if devResult.ManualHint == "" {
		t.Errorf("julian-ai DevBuild ManualHint must be non-empty")
	}
...
// A DevBuild tool (e.g. julian-ai with version="dev") MUST appear in UpgradeReport.Results
...
func TestExecute_DevBuildSurfacedAsSkipped(t *testing.T) {
	...
	// julian-ai (DevBuild) MUST appear in results as UpgradeSkipped.
	var devResult *ToolUpgradeResult
	for i := range report.Results {
		if report.Results[i].ToolName == "julian-ai" {
			r := report.Results[i]
	...
	if devResult == nil {
		t.Fatalf("julian-ai DevBuild must appear in Results as UpgradeSkipped, but was not found")
	}
	...
	if devResult.ManualHint == "" {
		t.Errorf("julian-ai DevBuild ManualHint must be non-empty — should explain dev/source build")
	}
...
func TestExecute_ManualFallbackSurfacedAsSkippedNotFailed(t *testing.T) {
	...
	results := []update.UpdateResult{
		makeResult("julian-ai", update.UpdateAvailable, "1.0.0", "1.5.0", update.InstallBinary),
	}
	results[0].UpdateHint = "See https://github.com/Gentleman-Programming/julian-ai/releases"
