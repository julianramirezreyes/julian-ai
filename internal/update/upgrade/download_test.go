...
func TestFindBinaryInTar(t *testing.T) {
	...
	entries := []struct {
		...
		{"julian-ai_1.5.0_darwin_arm64/julian-ai", content}, // binary in subdir
	}
	...
	outPath := filepath.Join(t.TempDir(), "julian-ai")
	...
	err := downloadBinary(context.Background(), server.URL+"/release.tar.gz", "julian-ai", outPath)
	if err != nil {
		t.Fatalf("downloadBinary: %v", err)
	}
