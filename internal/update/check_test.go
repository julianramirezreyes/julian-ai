...
func TestDetectInstalledVersion(t *testing.T) {
	tests := []struct {
		...
		{
			name:         "julian-ai uses build var",
			tool:         ToolInfo{Name: "julian-ai", DetectCmd: nil},
			currentBuild: "1.5.0",
			wantVersion:  "1.5.0",
		},
		{
			name:         "julian-ai dev build",
			tool:         ToolInfo{Name: "julian-ai", DetectCmd: nil},
			currentBuild: "dev",
			wantVersion:  "dev",
		},
...
func TestFetchLatestRelease_GithubToken(t *testing.T) {
	...
	if gotUserAgent != "julian-ai-update-check" {
		t.Fatalf("User-Agent = %q, want %q", gotUserAgent, "julian-ai-update-check")
	}
}
...
func TestCheckAll(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		switch {
		case contains(path, "julian-ai"):
			release = githubRelease{TagName: "v1.5.0", HTMLURL: "https://github.com/Gentleman-Programming/julian-ai/releases/tag/v1.5.0"}
	...
	// julian-ai: 1.5.0 local == 1.5.0 remote → UpToDate
	assertResult(t, results[0], "julian-ai", UpToDate, "1.5.0", "1.5.0")
...
func TestCheckAll_NetworkError(t *testing.T) {
	...
	// julian-ai has no DetectCmd, so it gets currentBuildVersion "1.0.0" as local
	// but fetch fails → CheckFailed (it has a local version).
	if results[0].Status != CheckFailed {
		t.Fatalf("julian-ai status = %q, want %q", results[0].Status, CheckFailed)
	}
	if results[0].Err == nil {
		t.Fatalf("julian-ai expected error, got nil")
	}
...
func TestUpdateHint(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "julian-ai macOS",
			tool:    ToolInfo{Name: "julian-ai"},
			profile: system.PlatformProfile{OS: "darwin", PackageManager: "brew"},
			want:    "brew upgrade julian-ai",
		},
		{
			name:    "julian-ai linux",
			tool:    ToolInfo{Name: "julian-ai"},
			profile: system.PlatformProfile{OS: "linux", PackageManager: "apt"},
			want:    "curl -fsSL https://raw.githubusercontent.com/Gentleman-Programming/julian-ai/main/scripts/install.sh | bash",
		},
		{
			name:    "julian-ai windows",
			tool:    ToolInfo{Name: "julian-ai"},
			profile: system.PlatformProfile{OS: "windows", PackageManager: "winget"},
			want:    "irm https://raw.githubusercontent.com/Gentleman-Programming/julian-ai/main/scripts/install.ps1 | iex",
		},
		{
		...
		},
		{
			...
			tool:    ToolInfo{Name: "engram"},
			profile: system.PlatformProfile{OS: "linux", PackageManager: "apt"},
			want:    "julian-ai upgrade (downloads pre-built binary)",
		},
		{
			name:    "engram windows",
			tool:    ToolInfo{Name: "engram"},
...
func TestRegistryContents(t *testing.T) {
	...
	expected := map[string]struct {
		...
		"julian-ai": {owner: "Gentleman-Programming", repo: "julian-ai"},
	...
	// julian-ai must have nil DetectCmd.
	if Tools[0].DetectCmd != nil {
		t.Fatalf("julian-ai DetectCmd should be nil")
	}
...
func TestCheckAll_DevVersion(t *testing.T) {
	...
	// Override only the first tool (julian-ai) by running CheckAll with "dev".
	origTools := Tools
	t.Cleanup(func() {
		httpClient = origClient
		lookPath = origLookPath
	...
	// Restrict to just julian-ai to isolate the test.
	...
	if results[0].Status != DevBuild {
		t.Fatalf("julian-ai dev status = %q, want %q", results[0].Status, DevBuild)
	}
}
...
// and that the dev-build sentinel causes julian-ai to be reported as DevBuild.
...
// when the running julian-ai binary reports version "dev", it is identified as a
// DevBuild and NOT reported as UpdateAvailable or VersionUnknown.
//
// The spec says:
//   - Dev build MUST be reported as development-build semantic
//   - julian-ai self-upgrade is skipped while engram/gga remain eligible
func TestCheckFiltered_DevBuildSemanticsForGentleAI(t *testing.T) {
	...
	lookPath = func(string) (string, error) { return "", fmt.Errorf("not found") }
	execCommand = func(name string, args ...string) *exec.Cmd { return exec.Command("false") }
	Tools = []ToolInfo{Tools[0]} // julian-ai only
	...
	r := results[0]
	if r.Tool.Name != "julian-ai" {
		t.Fatalf("tool = %q, want julian-ai", r.Tool.Name)
	}
...
// julian-ai with "dev" version gets DevBuild while engram with a real version stays eligible.
func TestCheckFiltered_DevBuildSkipNotEligible(t *testing.T) {
	...
	// Only julian-ai and engram for this test
	...
	// julian-ai should be DevBuild
	if results[0].Status != DevBuild {
		t.Fatalf("julian-ai status = %q, want DevBuild", results[0].Status)
	}
...
func TestNoUpdatesPath(t *testing.T) {
	...
	// Only engram and gga for this test (skip julian-ai to avoid dev-build behavior)
